...
void changeSpeed(float x, float y, float* vx, float* vy, POINT p)
{
    float v = sqrt(*vx * *vx + *vy * *vy);
//    float alpha = atan((p.y - 85 - y)/(p.x - 128 - x));
//    std::cout << v << "   " << alpha << "   ";
...
//    std::cout << "Вы будете играть красным шариком и ваша цель не дать двум шарикам вас догнать.";
    ...
    while (true)
    {
        ...
        //std::cout << p.x - 128 << " " << p.y - 85 << " ";

        changeSpeed(x1, y1, &vx1, &vy1, p);
        changeSpeed(x2, y2, &vx2, &vy2, p);
        changeSpeed(x3, y3, &vx3, &vy3, p);
